...
const ChatLeads: React.FC = () => {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/admin/chat/leads?${queryParams}`, {
        credentials: 'include'
      });
  ...
  const updateLeadStatus = async (leadId: string, status: string) => {
    try {
      const response = await fetch(`/api/v1/admin/chat/leads/${leadId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const exportLeads = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/admin/chat/export?${queryParams}`, {
        credentials: 'include'
      });
